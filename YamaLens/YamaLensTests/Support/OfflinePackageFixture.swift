...
struct OfflinePackageFixture {
    ...
    private func createCatalog(compressedBytes: Int, uncompressedHash: String) throws {
        ...
        CREATE TABLE mountain_points_of_interest(mountain_id TEXT NOT NULL REFERENCES mountains(id), point_of_interest_id TEXT NOT NULL REFERENCES points_of_interest(id), display_order INTEGER NOT NULL CHECK(display_order >= 0), PRIMARY KEY(mountain_id, point_of_interest_id));
        CREATE TABLE trailhead_access_points(trailhead_id TEXT NOT NULL REFERENCES points_of_interest(id), point_of_interest_id TEXT NOT NULL REFERENCES points_of_interest(id), display_order INTEGER NOT NULL CHECK(display_order >= 0), PRIMARY KEY(trailhead_id, point_of_interest_id));
        CREATE TABLE trailhead_search_areas(id TEXT PRIMARY KEY, trailhead_id TEXT NOT NULL REFERENCES points_of_interest(id), name TEXT NOT NULL, display_order INTEGER NOT NULL CHECK(display_order >= 0));
