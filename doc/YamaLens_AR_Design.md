...
8. ユーザーが候補を選び「記録する」を明示操作した場合だけ、写真と任意メモを「気になった山」として端末内へ保存する
...
- 専用の「方位を調整」操作で補正モードへ入り、一本指の横ドラッグでラベル全体を左右に移動する
- 通常のカメラ操作との誤認を避けるため、横ドラッグは補正モード中だけ有効にする
- 補正モード中は「手動調整中」と明示する
- 補正角を「東へ4°」などと表示する
- 「自動に戻す」で補正をリセットできるようにする
- ドラッグ操作が難しい利用者向けに、「西へ1°」「東へ1°」の単一タップ操作も用意し、長押し中は1°刻みで連続補正する
...
- 原則としてARセッション単位で保持する
- カメラ候補から開いた山詳細を直接閉じる往復では保持し、ホームまたはマイへ離れた時、アプリが非アクティブになった時、明示リセット時に破棄する
...
- 設定画面への案内を表示する
- 古い位置で表示を続けず、一時停止する
- カメラ利用中に位置観測が60秒を超えた場合は現在地を自動再取得し、成功後に候補算出を再開する

### 18.2 方位が不正確

- 精度低下を明示する
- 周囲の金属や磁石付きケースから離れるよう案内する
- 金属や磁石から離した後も復帰しない場合に備え、端末をゆっくり上下左右に動かして方位の再取得を促す操作を画面内で案内する
- 手動方位補正を提案する
- 方位観測が無効または3秒を超えて古い場合は古い候補を消し、有効値の再取得後にユーザー操作なしで候補算出を再開する
- 利用可能な真北方位へ戻った時点でARKitの方位基準を再同期する。復帰待ちの間に現在地が15秒を超えた場合は先に再取得し、方位復帰後の待ち時間を重ねない
...
- 「気になった山」の撮影は、AR候補を選択した後の明示操作だけで開始する。候補の表示、お気に入りの切替、詳細表示で写真を自動保存しない
- 写真はアプリ専用領域に保存し、連続したカメラ映像、写真ライブラリ、通常ログ、診断ログ、端末外へは保存・送信しない
- 正確な視点位置は、記録ごとに利用者が明示して有効化した場合だけ保存する。保存しない場合も撮影・記録を利用でき、位置履歴として常時保存しない
...
- 山頂・DEM・出典だけを持つ開発用ARテストパックを山域単位で生成し、天候と訪問機会に応じて複数山域の現地ログを収集する
...
- 開発中の識別性はARテストパックを用意した複数山域で評価し、正本で定める初期正式対応の合格には丹沢の代表地点を少なくとも1地点含め、目視可能な主要ピークを候補一覧に安定して提示できることとする
...
MVPの画面座標安定化は、1pt以内を静止ノイズとして保持し、64pt以上を意図的な移動として即時追従する。その中間は移動量に応じて時定数を0.30秒から0.06秒へ短くする。観測間隔が0.50秒を超えた場合、位置更新、手動方位補正、センサー利用不能時は、古い座標へ引っ張られないよう状態をリセットする。
...
オフライン利用は、正本どおり山域単位のパックを手動導入する。開発ビルドでは
山頂・DEM中心のARテストパックを複数山域へ追加できるが、正式対応とは数えない。
全国一括搭載や現在地半径だけによる配布はMVPの対象外とする。
...
天候によって一つの山域の検証が長期間できない場合に備え、正式対応地域とは別に
開発用ARテストパックを複数山域へ用意し、晴天と訪問機会に合わせて記録地点を
選べるようにする。丹沢外のログはアルゴリズムの検証に利用するが、丹沢正式対応の
最終証拠を置き換えない。現地待ちの間も、固定リプレイと周辺機能の開発を継続する。
...
初期実装は5Hzで記録し、直前10秒をメモリ内循環バッファとして保持する。
1セッションは最大1,500サンプル（最大5分）とし、上限到達後は保存または破棄を
促す。これらの値は `CameraDiagnosticPolicy` に集約し、変更時は正本文書と境界
テストを同時に更新する。
...
- JSONまたはNDJSONの同期ログ
- 問題発生マーカー
- Debugビルド限定の明示的な記録開始・保存・破棄
- `NSFileProtectionComplete`、バックアップ対象外、30日・20件上限、保持指定、個別・全削除
- カメラ画像を使わず、記録時と現在の候補位置を比較する端末内リプレイ
